...
* `numberwang(x)` for creating numeric values which present themselves as words.
* `uint16(x)` for creating 16-bit unsigned integers, where each value is
   stored as a pair of raw bytes.
* `fixedpt(x, decimals = 3)` for creating 32-bit fixed point numeric values.
...
[`{vctrs}` package](https://vctrs.r-lib.org/index.html), and its capabilities 
for easily creating new types, with minimal effort, along with broad support 
for standard R features.

These are all pretty naive implementations with ill-defined overflow, and 
...
You can install from [GitHub](https://github.com/coolbutuseless/typhoid) with:
...
The **numberwang** type is backed by the [`{numberwang}`](https://github.com/coolbutuseless/numberwang) package which provides
...
This offers space savings compared with R's standard integer (four bytes), as long
as your numbers fit in the given range.
